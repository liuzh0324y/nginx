        ...
        void *data, size_t len, ngx_uint_t escape)
{
    ...
    real_len = escape
        ? len + ngx_escape_html(NULL, (u_char*)data, len)
        : len;
    ...
    if (escape) {
        b->last = (u_char *)ngx_escape_html(b->last, (u_char*)data, len);
    } else {
        ...
        ngx_rtmp_live_app_conf_t *lacf)
{
    ...
    slcf = (ngx_rtmp_stat_loc_conf_t*)ngx_http_get_module_loc_conf(r, ngx_rtmp_stat_module);
    ...
    for (n = 0; n < lacf->nbuckets; ++n) {
        for (stream = lacf->streams[n]; stream; stream = stream->next) {
            ...
            for (ctx = stream->ctx; ctx; ctx = ctx->next, ++nclients) {
                ...
                if (ctx->publishing) {
                    codec = (ngx_rtmp_codec_ctx_t*)ngx_rtmp_get_module_ctx(s, ngx_rtmp_codec_module);
                }
            }
        ...
        ngx_rtmp_core_app_conf_t *cacf)
{
    ...
    if (slcf->stat & NGX_RTMP_STAT_LIVE) {
        ngx_rtmp_stat_live(r, lll,
                (ngx_rtmp_live_app_conf_t*)(cacf->app_conf[ngx_rtmp_live_module.ctx_index]));
    }

    if (slcf->stat & NGX_RTMP_STAT_PLAY) {
        ngx_rtmp_stat_play(r, lll,
                (ngx_rtmp_play_app_conf_t*)(cacf->app_conf[ngx_rtmp_play_module.ctx_index]));
    }
        ...
        ngx_rtmp_core_srv_conf_t *cscf)
{
    ...
    cacf = (ngx_rtmp_core_app_conf_t**)(cscf->applications.elts);
...
ngx_rtmp_stat_handler(ngx_http_request_t *r)
{
    ...
    cscf = (ngx_rtmp_core_srv_conf_t**)(cmcf->servers.elts);
...
ngx_rtmp_stat_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf = (ngx_rtmp_stat_loc_conf_t*)ngx_pcalloc(cf->pool, sizeof(ngx_rtmp_stat_loc_conf_t));
    if (conf == NULL) {
        return NULL;
    }
...
{
    ngx_rtmp_stat_loc_conf_t       *prev = (ngx_rtmp_stat_loc_conf_t*)parent;
    ngx_rtmp_stat_loc_conf_t       *conf = (ngx_rtmp_stat_loc_conf_t*)child;
